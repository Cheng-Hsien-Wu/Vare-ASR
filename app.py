...
class VareApp:
    ...
    async def _async_retry_llm(self, task: TranscriptionTask, output_path: Path) -> None:
        ...
        try:
            ...
            # Prepare Unified Pipeline Config
            # Crucial: This ensures manual retry uses the exact same logic as auto-correction
            # including Audio Grounding, Chunking, and File Caching.
            
            # Identify Audio Input Path
            # Task input path is usually the media file
            audio_input_path = task.input_path
            ...
            pipeline_config = {
                ...
                # Chunking limits (respect user setting or default)
                'llm_max_tokens': 60000, # Default safe limit, usually pipeline uses this or custom
            }
            ...
            def on_progress(idx, status):
                ...
                # Note: status might be a localized string from provider now
                task.status = status
                EventBus.emit(Events.TASK_STATUS_CHANGED, (self.tasks.index(task), task.status))
                
            def on_log(msg):
                # Bridge pipeline logs to app logger
                if isinstance(msg, tuple):
                    # Handle tuple keys if pipeline sends them (e.g. ('log_error', ...))
                    # For simplicty, just stringify
                    self._log(str(msg))
                else:
                    self._log(f"[AI] {msg}")
            ...
            # Execute Pipeline in Thread (it is blocking)
            # Pipeline.run expects input_path (the file to correct) and task_index
            def do_pipeline_run():
                # We pass the OUTPUT path (the SRT/TXT generated by transcription) as the input for correction
                pipeline.run(str(output_path), self.tasks.index(task))
